            ...
            if (text.capitalize() !in users) {
                if (text.length in 5..15) {
                    if (!hasNumber(text)) {
                        ...
                        Toast.makeText(
                            this,
                            "The username cannot contain special characters or spaces",
                            Toast.LENGTH_LONG
                        ).show()
                    }
                    Toast.makeText(this, "The username cannot contain numbers", Toast.LENGTH_LONG)
                        .show()
                }
                Toast.makeText(
                    this,
        ...
        private fun passwordAccepted(text: String): Boolean {
            if (text.length >= 8) {
                if (hasUpper(text)) {
                    ...
                    Toast.makeText(this, "The password must contain a number", Toast.LENGTH_LONG)
                        .show()
                }
                Toast.makeText(
                    this,
